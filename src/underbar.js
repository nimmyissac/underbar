(function() {
  ...
  _.filter = function(collection, test) {
    ...
    if(Array.isArray(collection)){
    for(var i=0;i<collection.length;i++){
    if(typeof(test(collection[i],i,collection)) === "boolean" && test(collection[i],i,collection)){
 ...
 return result;
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection,function(element,index,collection){
       tempCollection.push(iterator(element, index, collection));
    });
  ...
  _.contains = function(collection, target) {
    ...
    var keyArray = (Array.isArray(collection)) ? collection : Object.keys(collection);
    return _.reduce(keyArray, function(wasFound, item) {
    if (wasFound) {
      return true;
    }
    return (Array.isArray(collection)) ? item === target : collection[item] === target ;
   },false);
  };
  ...
  // var isFunction = (iterator === undefined) ? false : true;
   ...
   return _.reduce(collection,function(passed,element){
     ...
     var result = ((iterator(element) === null) || (iterator(element) === undefined) || (iterator(element) === NaN) || (iterator(element) === 0) || (iterator(element) ==="")) ? false : iterator(element);
 ...
 /* _.memoize = function(func) {
   ...
   return function(){
    ...
    for(var key in arg){
      if(arg[key] instanceof Array) isArray = true;
     }
    if(isArray){ // adding an extra argument to distinguish it from nonArray arguments.
     arg.push("Array");
    }
    if(arg in memo){
     return memo[arg];
 ...
 _.memoize = function(func) {
   ...
   return function(){
    var arg = Array.prototype.slice.call(arguments);
    var memoKey = JSON.stringify(arg);
    if(memoKey in memo) return memo[memoKey];
    else{
     memo[memoKey] = func.apply(this,arg);
     return memo[memoKey];
  }
 }
  ...
  _.shuffle = function(array) {
   ...
   while(newArr.length < array.length){
    ...
    if(!(index in filledIndex)){
     filledIndex[index] = true;
  ...
  _.zip = function() {
    ...
    var result =[];
    var length = _.reduce(args, function(result,element){
      return (element.length > result) ? element.length: result;
    },0);
    for(var index = 0; index < length; index++){
    var tempArray =[];
    for(var iteration = 0; iteration < count; iteration++){
      tempArray.push(args[iteration][index]);
    }
